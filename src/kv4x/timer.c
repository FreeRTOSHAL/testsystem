...
struct timer *timer;
#ifndef CONFIG_TIMER_TEST_KV4X_PWM
struct gpio_pin **rgb;
#else
struct pwm *pwm[3];
#endif
#ifdef CONFIG_CAPTURE_KV4X_TEST
...
# ifdef CONFIG_TIMER_TEST_KV4X_ONESHOT
	if (on) {
...
int32_t timertest_init(struct gpio_pin **rgbPins) {
	...
	timer = timer_init(FLEXTIMER1_ID, 128, MAX_TIME, 0);
	CONFIG_ASSERT(timer != NULL);
	...
	gpioPin_setPin(rgb[2]);
#else
	pwm[0] = pwm_init(FLEXTIMER0_PWM1_PTA4_ID);
	CONFIG_ASSERT(pwm[0]);
	pwm[1] = pwm_init(FLEXTIMER0_PWM2_PTA5_ID);
	CONFIG_ASSERT(pwm[1]);
	pwm[2] = pwm_init(FLEXTIMER1_PWM0_PTA12_ID);
	CONFIG_ASSERT(pwm[2]);
#endif
#ifndef CONFIG_TIMER_TEST_KV4X_PWM
# ifdef CONFIG_TIMER_TEST_KV4X_ONESHOT
...
# endif
	...
	CONFIG_ASSERT(pwm_setPeriod(pwm[1], MAX_TIME) == 0);
	CONFIG_ASSERT(pwm_setPeriod(pwm[2], MAX_TIME) == 0);
#endif
#ifdef CONFIG_CAPTURE_KV4X_TEST
